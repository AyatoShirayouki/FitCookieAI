...
namespace FitCookieAI_API.Controllers.Others
{
    ...
    [ApiController]
    public class PaymentPlansToUsersController : ControllerBase
    {
        ...
        public PaymentPlansToUsersController(IOptionsMonitor<JwtConfig> optionsMonitor, TokenValidationParameters tokenValidationParams, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<IActionResult> GetAll([FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (token != null && refreshToken != null)
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await PaymentPlansToUsersManagementService.GetAll();
        ...
        [HttpPost]
        [Route("Save")]
        public async Task<IActionResult> Save(PaymentPlanToUserDTO PaymentPlanToUserDTO, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            if (PaymentPlanToUserDTO == null || token == null || refreshToken == null)
            {
                response.Code = 400;
                response.Error = "Missing data - PaymentPlanToUser and/or token and/or refresh token is null or invalid!";
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await PaymentPlansToUsersManagementService.Save(PaymentPlanToUserDTO);
                    response.Body = "PaymentPlanToUser has been saved!";
        ...
        public async Task<IActionResult> GetById(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    response.Body = await PaymentPlansToUsersManagementService.GetById(id);
        ...
        public async Task<IActionResult> Delete(int id, [FromHeader] string token, [FromHeader] string refreshToken)
        {
            ...
            else
            {
                ...
                if (jwtUserToken.JwtSuccess == true)
                {
                    ...
                    await PaymentPlansToUsersManagementService.Delete(id);
                    response.Body = "PaymentPlanToUser has been deleted!";
